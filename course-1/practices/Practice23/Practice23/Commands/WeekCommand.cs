...
namespace Practice23.Commands
{
    public class WeekCommand: ICommand
    {
        ...
        public async Task ExecuteAsync(Update update, ITelegramBotClient botClient, CancellationToken ct)
        {
            ...
            if (parts.Length < 2)
            {
                await botClient.SendTextMessageAsync(chatId, "Использование: /week [группа]\nНапример: /week 9A", cancellationToken: ct);
                return;
            }
            var groupName = parts[1].Trim();
            var schedule = _scheduleRepository.Load();
            var group = schedule.Groups.FirstOrDefault(global => string.Equals(global.Group, groupName, StringComparison.OrdinalIgnoreCase));

            if (group != null)
            {
                await botClient.SendTextMessageAsync(chatId, $"Для группы {groupName} нет расписания.", cancellationToken: ct);
                return;
            }
            ...
            foreach (var day in group.Days)
            {
                ...
                {
                    lines.AddRange(day.Lessons.Select((l, i) => $"{i + 1}. {l.Time} -- {l.Subject} {(string.IsNullOrEmpty(l.Teacher) ? "" : "(" + l.Teacher + ")")}"));
                }
            }
